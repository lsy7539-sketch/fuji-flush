...
const BEGINNER_INTRO_STEPS = [
  ...
  "🤝 연합이 뭐냐면 — 테이블에 이미 3이 나와 있는데 나도 3을 내면, 그 둘이 연합해요! 힘(POWER)은 [숫자 × 인원수]라서 3×2=6이 되고, 이 6이 다른 낮은 카드나 연합을 밀어내는 힘이 돼요. 다른 플레이어가 테이블에 있는 숫자와 똑같은 카드를 내는 걸 보면, 바로 이 연합을 노리는 거예요.",
  "연합은 실제로 낸 숫자가 같아야만 돼요 — 계산된 POWER 값이 우연히 같다고 연합되는 건 아니에요. 손패를 가장 먼저 다 없애면 승리!",
  "카드를 하나 골라볼까요? 🎴",
];
